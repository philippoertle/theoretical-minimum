...
2. **Reversible** — each state has exactly one predecessor (the same law read
   backward is deterministic), so *f* is a **bijection** of ℤ.

Equation (2) in the book is *N(n+1) = N(n) − 1* (the “minus-one” step), not
another copy of +1.
"""
...
# Book equations (2)–(5) for integer state N(n) ∈ ℤ.
...
def step_eq5_sign_flip(n: int) -> int:
    """Eq. (5): N(n+1) = (−1)^N(n) N(n), using parity on ℤ.
