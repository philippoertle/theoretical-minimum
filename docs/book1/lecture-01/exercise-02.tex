...
exactly one successor.\footnote{A \emph{state} is one allowed configuration of
the system. A \emph{successor} is the next state after one update step.}
...
classified, up to relabeling, by their cycle type\footnote{A \emph{cycle type}
records lengths of disjoint cycles in a permutation, e.g. \((3,2,1)\).}
(equivalently, a partition of
6\footnote{A \emph{partition} of an integer is a sum of positive integers where
order is ignored, e.g. \(6=3+2+1\).}).
