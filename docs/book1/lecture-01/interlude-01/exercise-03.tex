...
\]
Here \(\lVert\mathbf{A}\rVert\) denotes Euclidean norm\footnote{Also called the
...
\]
The dot product\footnote{For \(\mathbf{u},\mathbf{v}\in\mathbb{R}^n\),
\(\mathbf{u}\cdot\mathbf{v}=\sum_i u_i v_i\).} is the standard inner product on
\(\mathbb{R}^n\)\footnote{\(\mathbb{R}^n\) is the set of \(n\)-component real
vectors.}.
Hence,
\[
